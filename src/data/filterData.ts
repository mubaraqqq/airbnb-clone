import trending from "@/assets/trending.jpg";
import views from "@/assets/views.jpg";
import omg from "@/assets/omg.jpg";
import breakfast from "@/assets/breakfast.jpg";
import mansions from "@/assets/mansions.jpg";
import rooms from "@/assets/rooms.jpg";
import cities from "@/assets/cities.jpg";
import design from "@/assets/design.jpg";
import domes from "@/assets/domes.jpg";
import pools from "@/assets/pools.jpg";
import boats from "@/assets/boats.jpg";
import kitchens from "@/assets/kitchen.jpg";
import desert from "@/assets/desert.jpg";
import earth from "@/assets/earth.jpg";
import beachfront from "@/assets/beachfront.jpg";
import islands from "@/assets/islands.jpg";
import top from "@/assets/top.jpg";
import cycladic from "@/assets/cycladic.jpg";
import tropical from "@/assets/tropical.jpg";
import historical from "@/assets/historical.jpg";
import countryside from "@/assets/countryside.jpg";
import parks from "@/assets/parks.jpg";
import play from "@/assets/play.jpg";
import casas from "@/assets/casas.jpg";
import ski from "@/assets/ski.jpg";
import beach from "@/assets/beach.jpg";
import cabins from "@/assets/cabins.jpg";
import towers from "@/assets/towers.jpg";
import lakefront from "@/assets/lakefront.jpg";
import farms from "@/assets/farms.jpg";
import vineyards from "@/assets/vineyards.jpg";
import arctic from "@/assets/arctic.jpg";
import houseboats from "@/assets/houseboats.jpg";
import caves from "@/assets/caves.jpg";
import surfing from "@/assets/surfing.jpg";
import tiny from "@/assets/tinyhomes.jpg";
import pianos from "@/assets/pianos.jpg";
import shepherd from "@/assets/shepherd.jpg";
import ryokans from "@/assets/ryokans.jpg";
import lake from "@/assets/lake.jpg";
import news from "@/assets/new.jpg";
import yurts from "@/assets/yurts.jpg";
import camping from "@/assets/camping.jpg";
import offGrid from "@/assets/offgrid.jpg";
import castles from "@/assets/castles.jpg";
import aframes from "@/assets/a-frames.jpg";
import luxe from "@/assets/luxe.jpg";
import treehouses from "@/assets/treehouses.jpg";
import campers from "@/assets/campers.jpg";
import golfing from "@/assets/golfing.jpg";
import skiing from "@/assets/skiing.jpg";
import hanoks from "@/assets/hanoks.jpg";
import windmills from "@/assets/windmills.jpg";
import minsus from "@/assets/minsus.jpg";
import barns from "@/assets/barns.jpg";
import adapted from "@/assets/adapted.jpg";
import containers from "@/assets/containers.jpg";
import creative from "@/assets/creative.jpg";
import dammusi from "@/assets/dammusi.jpg";
import riads from "@/assets/riads.jpg";
import trulli from "@/assets/truli.jpg";
import { FilterItem } from "@/types";

export const filterArrayData: FilterItem[] = [
  { label: "Trending", icon: trending },
  { label: "Amazing views", icon: views },
  { label: "OMG!", icon: omg },
  { label: "Bed & breakfast", icon: breakfast },
  { label: "Mansions", icon: mansions },
  { label: "Rooms", icon: rooms },
  { label: "Amazing pools", icon: pools },
  { label: "Iconic cities", icon: cities },
  { label: "Design", icon: design },
  { label: "Domes", icon: domes },
  { label: "Boats", icon: boats },
  { label: "Chef's kitchens", icon: kitchens },
  { label: "Desert", icon: desert },
  { label: "Earth homes", icon: earth },
  { label: "Beachfront", icon: beachfront },
  { label: "Islands", icon: islands },
  { label: "Top of the world", icon: top },
  { label: "Cycladic homes", icon: cycladic },
  { label: "Tropical", icon: tropical },
  { label: "Historical homes", icon: historical },
  { label: "Countryside", icon: countryside },
  { label: "National parks", icon: parks },
  { label: "Play", icon: play },
  { label: "Casas particulares", icon: casas },
  { label: "Ski-in/out", icon: ski },
  { label: "Beach", icon: beach },
  { label: "Cabins", icon: cabins },
  { label: "Towers", icon: towers },
  { label: "Lakefront", icon: lakefront },
  { label: "Farms", icon: farms },
  { label: "Vineyards", icon: vineyards },
  { label: "Arctic", icon: arctic },
  { label: "Houseboats", icon: houseboats },
  { label: "Caves", icon: caves },
  { label: "Surfing", icon: surfing },
  { label: "Tiny homes", icon: tiny },
  { label: "Grand pianos", icon: pianos },
  { label: "Shepherd's huts", icon: shepherd },
  { label: "Ryokans", icon: ryokans },
  { label: "Lake", icon: lake },
  { label: "New", icon: news },
  { label: "Yurts", icon: yurts },
  { label: "Camping", icon: camping },
  { label: "Off-the-grid", icon: offGrid },
  { label: "Castles", icon: castles },
  { label: "A-frames", icon: aframes },
  { label: "Luxe", icon: luxe },
  { label: "Treehouses", icon: treehouses },
  { label: "Campers", icon: campers },
  { label: "Golfing", icon: golfing },
  { label: "Skiing", icon: skiing },
  { label: "Hanoks", icon: hanoks },
  { label: "Windmills", icon: windmills },
  { label: "Minsus", icon: minsus },
  { label: "Barns", icon: barns },
  { label: "Adapted", icon: adapted },
  { label: "Containers", icon: containers },
  { label: "Creative spaces", icon: creative },
  { label: "Dammusi", icon: dammusi },
  { label: "Riads", icon: riads },
  { label: "Trulli", icon: trulli },
];
